...
begin ;
-- v10 partitioning test

CREATE TABLE test.testfk10 ( id bigint primary key ) ;


CREATE  TABLE  IF NOT EXISTS test.testpart
(
   id bigserial,
...
insert into partmgr.part_table (schemaname, tablename, keycolumn, pattern, cleanable, detachable, retention_period)  values
          ('test','testpart','dateev','M','f','t', '6 month');
insert into partmgr.part_index (schemaname, tablename, indexdef)  values
          ('test','testpart','create index on test.testpart ( dateev )'),
          ('test','testpart','create index on test.testpart ( n )') ;

insert into partmgr.part_fkey (schemaname, tablename, fkeydef)  values
          ('test','testpart','alter table test.testpart add foreign key ( id ) references test.testfk10( id )');

insert into partmgr.part_trigger (schemaname, tablename, triggername, triggerdef)  values
          ('test','testpart','_delev','create trigger _delev after delete on test.testpart for each row execute procedure test.test_trigger()');

select * from partmgr.create('test','testpart', (current_date - interval '12 month')::date  , (current_date + interval '3 month')::date );
